...
from Modules.M9_Operation_Strategy.M9_0_Charging_Management_Multiday.M9_0_2_Over_Night import M902_Overnight_CM
from Modules.M10_Evaluation import M10_Evaluation_Aging
import numpy as np
# Used Sources in this File:


def opsim_multiday(scenario, eol):
    ...
    # Vehicle Battery Capacity Update in EOL Simulation
    vehicle.battery_capacity = eol.battery_capacity_initial_kWh * eol.battery_soh
    ...
    sim = initialization(env, sim, scenario, eol)
    ...
    while sim.step_dis < sim.dis_steps - 1:
        # Driving Simulation
        sim = M82_DrivingSim.driving_sim(sim, vehicle, env)
        # Overnight Simulation
        if sim.step_dis < sim.dis_steps - 1:
            sim = M902_Overnight_CM.overnight_charging(sim, env, vehicle)
    #print('Finished(:-)')
    ...
    return sim, env, vehicle, results
...
def initialization(env, sim, scenario, eol):
    ...
    sim.daytime = env.freight_mission_start  # mission start from scenario
    ...
    sim.pos_poi_max = 4000000  # set initial to 4000 km
    # Aging Evaluation yes (1) or no (0)
    ...
    # Overnight Stay tracking time in h
    sim.betos_overnight_stay_time_dis = np.zeros(sim.dis_steps)
    # Overnight Stay tracking position in meter
    sim.betos_overnight_stay_pos_dis = np.zeros(sim.dis_steps)
    ...
    # Number of aging events
    number_aging_events = 100
    # Index for tracking aging evaluation events
    sim.eol_aging_evaluation_event = 1
    # SOC Values at aging evaluation
    sim.eol_battery_soc_value = np.zeros(number_aging_events)
    sim.eol_battery_soc_value[0] = env.freight_start_soc
    # SOH Value at aging evaluation
    sim.eol_battery_soh = np.zeros(number_aging_events)
    sim.eol_battery_soh[0] = eol.battery_soh
    # DOD Values at every aging evaluation
    sim.eol_battery_dod = np.zeros(number_aging_events)
    # Average SOC Values at every aging evaluation
    sim.eol_battery_soc_avg = np.zeros(number_aging_events)
    # Average C Rate
    sim.eol_battery_crate_mean = np.zeros(number_aging_events)
    # Max C Rate
    sim.eol_battery_crate_max = np.zeros(number_aging_events)
    # Mean Cell Temperature
    sim.eol_battery_temp_mean = np.zeros(number_aging_events)
    # Aging Event ID (1: On Route, 2: Overnight Weekday, 3: Weekend Charging, 0: Driving)
    sim.eol_battery_aging_event_id = np.zeros(number_aging_events)

    # Temperature at aging evaluation
    sim.eol_battery_temperature_cell = np.zeros(number_aging_events)
    sim.eol_battery_temperature_cell[0] = 20  # in Grad Celsius
    sim.eol_battery_temperature_housing = np.zeros(number_aging_events)
    sim.eol_battery_temperature_housing[0] = 20  # in Grad Celsius
    # Internal Resistance at aging evaluation
    sim.eol_battery_internal_resistance_cyclic = np.zeros(number_aging_events)
    sim.eol_battery_internal_resistance_cyclic[0] = eol.battery_internal_resistance_cyclic
    sim.eol_battery_internal_resistance_calendaric = np.zeros(number_aging_events)
    sim.eol_battery_internal_resistance_calendaric[0] = eol.battery_internal_resistance_calendaric
    # Capacity loss at aging evaluation
    sim.eol_battery_q_loss_cyclic = np.zeros(number_aging_events)
    sim.eol_battery_q_loss_cyclic[0] = eol.battery_q_loss_cyclic
    sim.eol_battery_q_loss_calendaric = np.zeros(number_aging_events)
    sim.eol_battery_q_loss_calendaric[0] = eol.battery_q_loss_calendaric

    # Position of aging evaluation
    sim.eol_battery_aging_pos = np.zeros(number_aging_events)
    # Time of aging evaluation
    sim.eol_battery_aging_time = np.zeros(number_aging_events)
    # Time step (index in time vector) of aging evaluation
    sim.eol_battery_aging_time_step = np.zeros(number_aging_events)

    return sim

# ______________________________________________________________________________________________________________________
# Building Wrapper for Single Simulation run
def single_sim_multiday(scenario):
    # _____________________________
    # Build EOL Object for Aging Investigation (either aging is not calculated)
    class EOL:
        pass
    eol = EOL()
    # Set battery parameters
    eol.battery_soh = 1
    eol.battery_capacity_initial_kWh = scenario['battery_capacity'][0]
    ...
    sim, env, vehicle, results = opsim_multiday(scenario, eol)
