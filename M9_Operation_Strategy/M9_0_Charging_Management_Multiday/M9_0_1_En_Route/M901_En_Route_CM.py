...
# BET.OS Function #
#
# Designed by MX-2023-07-02
# Last update: MX-2023-07-02
#
# Function Description
# Part of the Operation Strategy Module M9
# Part of the holistic operation strategy including En-route Fast Charging and Overnight Slow/BiDi Charging
# Dynamic Programming Operation Strategy with the handling of occupied charging stations for En-Route Fast Charging
# _________________________________
...
# Main Function based on DP
def betos_dynamic_programming_multi(sim, env, vehicle):
    # Searching critical PoI due to mandatory break
    sim = finding_critical_poi(sim, env, vehicle)
    # Composition Properties
    n_poi, poi_id_crit, poi_id_current, poi_id_max = composition(sim, vehicle, env)
    # Setting up state space
    soc_state, rest_time_state, cost_matrix, best_decision_charge_time, best_decision_rest_time = state_space_definition(
        sim, vehicle, env, n_poi, poi_id_crit, poi_id_current)
    ...
    for poi in range(n_poi - 2, -1, -1):
        ...
        for soc in soc_state:
            ...
            for rest in rest_time_state:
                ...
                next_state_soc, next_state_rest, cost_not_allowed_dec = next_state(sim, vehicle, env, decision_1_charge, decision_2_rest,
                                                             poi, soc, rest, dp_data, rest_int, poi_id_current)
                # lowest cumulative cost when taking best decision in current state
                cost_matrix, best_decision_charge_time, best_decision_rest_time = cost_next_state(sim, vehicle, env,
                                                                                                  next_state_soc,
                                                                                                  next_state_rest,
                                                                                                  soc_int, rest_int,
                                                                                                  decision_cost_map,
                                                                                                  cost_matrix, poi,
                                                                                                  best_decision_charge_time,
                                                                                                  best_decision_rest_time,
                                                                                                  soc_state, dp_data, cost_not_allowed_dec)
    ...
    rest_remain = vehicle.mandatory_rest_time - (
                sum(sim.rest_time_dis[sim.pos_rest_done + 1:sim.step_dis + 1]) / 60)  # in min
    if rest_remain <= 0 and (max(sim.rest_time_dis[sim.pos_rest_done + 1:sim.step_dis + 1]) / 60 >= 45 or
                             (max(sim.rest_time_dis[sim.pos_rest_done + 1:sim.step_dis + 1]) / 60 >= 30 and
                              max(sim.rest_time_dis[
                                      sim.rest_time_dis[sim.pos_rest_done + 1:sim.step_dis] / 60 < 30]) / 60 >= 15)):
    ...
    next_state_soc, next_state_rest, cost_not_allowed_dec = next_state(sim, vehicle, env, decision_1_charge, decision_2_rest, current_poi,
    ...
    if sim.betos_rest_time[sim.step_dis] > 0 or sim.betos_charge_time[sim.step_dis] > 0:
        ...
        sim.betos_poi_availability[sim.step_dis] = env.cs_data_availability(sim.daytime)[
            int(env.infra_array_poi_type[sim.step_dis])]
        # Track average power
        sim.betos_power[sim.step_dis] = env.cs_power
        # Track arrival time at POI
...
def dp_data_func(sim, env):
    ...
    for i in range(0, number_poi + 1):
        # Power
        dp_data[i, 0] = env.cs_data_power[
            int(env.infra_array_poi_type[int(env.infra_array_poi_pos[poi_id_current + i])])]
        # Time
        ...
        if i > 0:
            # Distance
            dp_data[i, 1] = env.infra_array_poi_pos[poi_id_current + i] - env.infra_array_poi_pos[
                poi_id_current + i - 1]
            # energy consumption (5 % Saftey)
            dp_data[i, 2] = (
                        sum(sim.betos_energy_cons_dis_prediction[int(env.infra_array_poi_pos[poi_id_current + i - 1]):
                                                                 int(env.infra_array_poi_pos[poi_id_current + i])]) / \
                        (1000 * 60 * 60))  # in kWh
            # Arrival time at PoI
            dp_data[i, 3] = dp_data[i - 1, 3] + (
                sum(sim.betos_time_dis_prediction[int(env.infra_array_poi_pos[poi_id_current + i - 1]):
                                                  int(env.infra_array_poi_pos[poi_id_current + i])])) / 3600  # in h
            # Limit Arrival Time to 22o clock (due to no daytime depending availability, this results not in an error)
            dp_data[i, 3] = min(22, dp_data[i, 3])

        # Availability of PoI
        dp_data[i, 4] = env.cs_data_availability(dp_data[i, 3])[
            int(env.infra_array_poi_type[int(env.infra_array_poi_pos[poi_id_current + i])])]
...
def finding_critical_poi(sim, env, vehicle):
    ...
    steps_max = min(len(time_prediction), (vehicle.max_day_distance - (sim.step_dis - sim.pos_overnight_stay)))  # maximum daily distance barrier
    # Index
    i = 0
    # Search for critical PoI if time remain is greater than zero
...
def composition(sim, vehicle, env):
    ...
    # ID of max POI at this day
    poi_id_max = int(env.infra_array_poi_id[sim.pos_poi_max])
    # ID of destination PoI
    poi_id_destination = int(env.infra_array_poi_id[-1])
    # n PoI for Consideration
    # n_poi = poi_id_destination - poi_id_current + 1
    n_poi = poi_id_max - poi_id_current + 1

    return n_poi, poi_id_crit, poi_id_current, poi_id_max
...
def next_state(sim, vehicle, env, decision_1_charge, decision_2_rest,
    ...
    # Interpolation of SOC: Calculation of SOC after reaching next POI under decision
    next_state_soc_floor = ((vehicle.charging_map[int(env.infra_array_poi_type[pos_type]), int(np.floor(soc * 100)),
                             :] / 100)) - (dp_data[poi + 1, 2] / vehicle.battery_capacity)
    next_state_soc_ceil = ((vehicle.charging_map[int(env.infra_array_poi_type[pos_type]), int(np.ceil(soc * 100)),
                            :] / 100)) - (dp_data[poi + 1, 2] / vehicle.battery_capacity)
    ...
    # Cost after decision at this POI
    condition_not_allowed_dec = next_state_soc + (dp_data[poi + 1, 2] / vehicle.battery_capacity)  > vehicle.battery_soc_max
    cost_not_allowed_dec = np.zeros(len(next_state_soc))
    # Add Costs for Decision leading to SOC over allowed SOC (array with length of next_state_SoC)
    cost_not_allowed_dec[condition_not_allowed_dec == True] = 10000  # in sec
    ...
    return next_state_soc, next_state_rest, cost_not_allowed_dec


# ______________________________________________________________________________________________________________________
# Calculation of Cost of taken decisions
def cost_next_state(sim, vehicle, env, next_state_soc, next_state_rest, soc_int, rest_int, decision_cost_map,
                    cost_matrix,
                    poi, best_decision_charge_time, best_decision_rest_time, soc_state, dp_data, cost_not_allowed_dec):
    ...
    for decision_2 in range(0, len(next_state_rest)):
        ...
        lan_cost_expected = (poi_availability * cost_of_soc_decision_free) + (
                    1 - poi_availability) * cost_of_soc_decision_occ
        ...
        cumulative_cost[:, decision_2] = (dec_cost_expected / 3600) + lan_cost_expected + (cost_not_allowed_dec / 3600)  # in h
...
def best_decision_fdp(sim, vehicle, env, soc_state, next_state_soc, next_state_rest, cost_matrix, decision_cost_map,
                      current_poi, soc, poi, poi_id_current, cost_not_allowed_dec):
    ...
    # Add costs in case of early full rest decision if driving time to daily destination from this POI is greater than
    # allowable driving time of 4.5 h
    add_decision_cost = np.zeros_like(decision_cost_map)
    if sum(sim.betos_time_dis_prediction[int(sim.step_dis):int(sim.pos_poi_max)]) / 3600 > 4.5:
    ...
    if poi_available == 0:
        ...
        for decision_2 in range(0, len(next_state_rest)):
            ...
            # Cost of decision
            cumulative_cost[:, decision_2] = ((decision_cost_map[:, decision_2] +
                                               add_decision_cost[:, decision_2]) / 3600) + \
                                               cost_of_soc_decision + (cost_not_allowed_dec / 3600) # in h
    ...
    # If poi is occupied
        ...
        for decision_2 in range(0, len(next_state_rest)):
            ...
            # Expected Cost of decision: Expected cost due to probability of PoI Availability
            dec_cost_expected = decision_cost_map[:, decision_2] + poi_wait_time
            # Expected Cost of landing state
